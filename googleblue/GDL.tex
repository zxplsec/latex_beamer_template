...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item 
      % You can easily create overlays, themes allow you to change the appearance of your presentation to suit your purposes.
       你可以轻松地创建排版方式、主题，从而允许你修改演示文稿的外观以适合你的意图。
    \item 
      % The layout, colors, and fonts used in a presentation can easily be changed globally, but you also have control over the most minute detail.
...
\begin{frame}
  ...
  \begin{itemize}
    \setlength{\itemsep}{8pt}
    \item 
      %Each theme is designed to be highly usable and readable. This makes the presentation more professional looking and easier for the audience to follow.
      设计主题时应做到使用性高、可读性强，这使得演示文稿更具专业性，同时让观众更容易理解。
    \item 
      %The final output is typically a \alert{.pdf} file. Viewer applications for this format exist for virtually every platform.
      输出一般为\alert{.pdf}文件。
    \item 
      % \alert{Your presentation will look exactly the same no matter which computer or viewer program is being used.}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item 
      ...
      每一个Beamer项目都是由一系列框构成的。
    \item 
      % Each frame produces one or more slides, depending on the slide's overlays, which will be discussed later.
      每一个框产生一个或多个幻灯片，这依赖于幻灯片的排版方式。
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item 
      % The alignment option is centered \texttt{[c]} by default. The values \texttt{[t]} (top align) and \texttt{[b]} (bottom align) are also accepted.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \setlength{\itemsep}{8pt}
    \item 
      % The \texttt{[plain]} option for the frame environment causes the headlines, footlines, and sidebars to be suppressed. This can be useful for showing large pictures.
...
\begin{frame}
  ...
  $\vcenter{\hbox{\includegraphics[width=1cm]{./images/GitHub-Mark-120px-plus.png}}}$
  \qquad
  $\vcenter{\hbox{\includegraphics[width=2cm]{./images/mactexlogoX5.png}}}$
...
\begin{frame}
  ...
  \begin{description}[labelindent=0pt, labelwidth=6cm]
    ...
    \item[\texttt{\textbackslash{}transsplitverticalin}] Sweeps two vertical lines in
    \item[\texttt{\textbackslash{}transsplitverticalout}] Sweeps two vertical lines out
    \item[\texttt{\textbackslash{}transsplithorizontalin}] Sweeps two horizontal lines in
    \item[\texttt{\textbackslash{}transsplithorizontalout}] Sweeps two horizontal lines out
